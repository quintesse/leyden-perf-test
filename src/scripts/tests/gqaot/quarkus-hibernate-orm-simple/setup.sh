...
sed 's/localhost:5434/localhost:5432/g' "${TEST_APPS_DIR}/${REPO_NAME}/quarkus-hibernate-orm-simple/src/main/resources/application.properties" > "${TEST_APPS_DIR}/${REPO_NAME}/quarkus-hibernate-orm-simple/src/main/resources/application.properties.new"
...
dest="${TEST_BUILDS_DIR}/${REPO_NAME}/quarkus-hibernate-orm-simple/quarkus-hibernate-orm-simple/"
pushd "$dest" > /dev/null
rm -rf "${dest:?}/db"
mkdir -p "$dest/db"
cp -a "${TEST_SRC_DIR}/scripts/tests/gqaot/quarkus-hibernate-orm-simple.sql" "$dest/db"
popd > /dev/null
