...
assert_case_output_matches_fixture() {
  ...
  if command -v timeout >/dev/null 2>&1; then
    timeout --foreground "${VERIFY_SETUP_TIMEOUT_SEC}" bash -c 'cd "$1" && ./run qdup -j 25 -d dummy -s "$2" -C tests-dummy "$3" -o "$4" > "$5" 2>&1' _ "${REPO_ROOT}" "${strategy}" "${test_id}" "${out_dir}" "${log_file}"
  else
    bash -c 'cd "$1" && ./run qdup -j 25 -d dummy -s "$2" -C tests-dummy "$3" -o "$4" > "$5" 2>&1' _ "${REPO_ROOT}" "${strategy}" "${test_id}" "${out_dir}" "${log_file}"
  fi
